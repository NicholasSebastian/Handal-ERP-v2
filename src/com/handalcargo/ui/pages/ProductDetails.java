...
public class ProductDetails extends Layout {
	...
	protected void onDelete(Object selected) {
		Database.update("DELETE FROM keteranganbrg WHERE brgcode = ?", statement -> {
			try {
	...
	class Form extends JPanel {
		...
		public void onSave() {
			Database.update("INSERT INTO keteranganbrg VALUES (?, ?)", 
			statement -> {
			try {
	...
	class ModifyForm extends Form {
		...
		@Override
		public void onSave() {
			Database.update("UPDATE keteranganbrg SET brgdesc = ? WHERE brgcode = ? ", 
				statement -> {
				try {
