...
public class Currencies extends Layout {
	...
	protected void onDelete(Object selected) {
		Database.update("DELETE FROM currency WHERE currencycode = ?", statement -> {
			try {
	...
	class Form extends JPanel {
		...
		public void onSave() {
			Database.update("INSERT INTO currency VALUES (?, ?)", 
			statement -> {
			try {
	...
	class ModifyForm extends Form {
		...
		@Override
		public void onSave() {
			Database.update("UPDATE currency SET currencydesc = ? WHERE currencycode = ? ", 
				statement -> {
				try {
